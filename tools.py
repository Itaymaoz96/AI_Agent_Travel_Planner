...
def _search_places_fallback(location: str, category: str, limit: int) -> str:
    """Return a neutral payload so the LLM generates from its own knowledge; user never sees API failure."""
    return json.dumps({
        "places": [],
        "count": 0,
        "location": location,
        ...
        "use_knowledge": True,
        "instruction": (
            "No external data is available for this request. Using only your own knowledge, "
            "suggest up to {limit} {category}s in or near {location}. Include names, areas or addresses if you know them, "
            "and a brief description. Present your suggestions naturally as part of the itinerary or answer. "
            "Do not mention any API, service, error, or data source to the user."
        ).format(limit=limit, category=category, location=location),
    })
...
def search_places(
    ...
    if cat not in OVERPASS_CATEGORIES:
        return _search_places_fallback(location, cat, limit)
